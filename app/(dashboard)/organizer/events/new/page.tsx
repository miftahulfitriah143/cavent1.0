  ...
  const nextStep = () => {
    if (step === 1) {
      ...
      if (!formData.maxCapacity || parseInt(formData.maxCapacity) <= 0) {
        toast.error("Mohon isi Kapasitas Maksimal dengan angka yang benar!");
        return;
      }
    ...
    if (step === 2) {
      ...
      if (!formData.benefits.trim()) {
        toast.error("Mohon isi Benefit Acara!");
        return;
      }
      if (!formData.targetAudience.trim()) {
        toast.error("Mohon isi Target Peserta!");
        return;
      }
      ...
      if (new Date(formData.regOpenDate) > new Date(formData.regCloseDate)) {
        toast.error("Tanggal Pendaftaran Tutup tidak boleh mendahului Tanggal Pendaftaran Buka!");
        return;
      }
      if (new Date(formData.regCloseDate) > new Date(formData.startDate)) {
        toast.error("Tanggal Pendaftaran Tutup tidak boleh melampaui Tanggal Acara Dimulai!");
        return;
      }
    }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-dark tracking-tight">Buat Acara Baru</h1>
          <p className="text-neutral text-sm mt-1">Isi informasi acara dengan lengkap dan akurat</p>
        </div>
      </div>
                ...
                <div className="h-64 border-2 border-dashed border-gray-200 rounded-[2.5rem] flex flex-col items-center justify-center gap-4 bg-gray-50/50 group-hover:bg-primary/5 group-hover:border-primary/20 transition-all">
                  {bannerPreview ? (
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-xs font-bold text-primary">{formData.bannerPoster?.name}</span>
                    </div>
                  ) : (
                    <>
