...
export default function NewEventPage() {
    ...
    const loadCategories = async () => {
      try {
        ...
        if (snap.empty) {
          ...
          // Ensure defaults always appear first
          const sorted = [
  ...
  const handleAddCustomCategory = async () => {
    ...
    if (!availableCategories.includes(formatted)) {
      ...
      // Save to Firestore so it persists for all users
      try {
  ...
  const removeAdditionalMedia = (idx: number) => {
    if (additionalMediaPreviews[idx]) {
      URL.revokeObjectURL(additionalMediaPreviews[idx]);
    }
    setFormData(prev => ({
      ...prev,
      additionalMedia: prev.additionalMedia.filter((_, i) => i !== idx)
    }));
  ...
  const handleSubmit = async () => {
    if (!formData.title || !formData.startDate || !formData.bannerPoster) {
      toast.error("Mohon lengkapi informasi wajib (Judul, Tanggal, & Poster)");
      return;
    }
    ...
    try {
      ...
      await addDoc(collection(db, "notifications"), {
        type: "NEW_EVENT_PROPOSAL",
        title: "Pengajuan Acara Baru",
        message: `${auth.currentUser.displayName || "Penyelenggara"} mengajukan acara baru: ${formData.title}`,
        eventId: eventRef.id,
        status: "unread",
        createdAt: serverTimestamp(),
        targetRole: "admin"
      ...
      toast.success("Acara berhasil diajukan! Menunggu persetujuan admin.");
  ...
  const nextStep = () => {
    if (step === 1) {
      if (!formData.title.trim()) { toast.error("Mohon isi Nama Acara!"); return; }
      if (!formData.startDate) { toast.error("Mohon isi Tanggal Mulai!"); return; }
      if (!formData.endDate) { toast.error("Mohon isi Tanggal Selesai!"); return; }
      if (!formData.startTime) { toast.error("Mohon isi Jam Mulai!"); return; }
      if (!formData.endTime) { toast.error("Mohon isi Jam Selesai!"); return; }
      if (formData.category.length === 0) { toast.error("Mohon pilih minimal satu Kategori!"); return; }
      if (formData.campusLocation !== 'Online' && !formData.venue.trim()) { toast.error("Mohon isi Ruangan / Venue!"); return; }
      if (formData.campusLocation === 'Online' && !formData.meetingLink.trim()) { toast.error("Mohon isi Link Zoom/GMeet!"); return; }
      if (!formData.maxCapacity || parseInt(formData.maxCapacity) <= 0) { toast.error("Mohon isi Kapasitas Maksimal dengan angka yang benar!"); return; }
      if (new Date(formData.startDate) > new Date(formData.endDate)) { toast.error("Tanggal Selesai tidak boleh mendahului Tanggal Mulai!"); return; }
    }

    if (step === 2) {
      if (!formData.description.trim()) { toast.error("Mohon isi Deskripsi Acara!"); return; }
      if (!formData.benefits.trim()) { toast.error("Mohon isi Benefit Acara!"); return; }
      if (!formData.targetAudience.trim()) { toast.error("Mohon isi Target Peserta!"); return; }
      if (!formData.regOpenDate) { toast.error("Mohon isi Tanggal Pendaftaran Buka!"); return; }
      if (!formData.regCloseDate) { toast.error("Mohon isi Tanggal Pendaftaran Tutup!"); return; }
      if (new Date(formData.regOpenDate) > new Date(formData.regCloseDate)) { toast.error("Tanggal Pendaftaran Tutup tidak boleh mendahului Tanggal Pendaftaran Buka!"); return; }
      if (new Date(formData.regCloseDate) > new Date(formData.startDate)) { toast.error("Tanggal Pendaftaran Tutup tidak boleh melampaui Tanggal Acara Dimulai!"); return; }
    }
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="bg-gradient-to-r from-primary-900 via-primary to-secondary rounded-2xl p-6 md:p-8 text-white relative overflow-hidden shadow-xl shadow-primary/20">
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-2xl md:text-3xl font-black tracking-tight">Buat Acara Baru</h1>
          <p className="text-white/70 text-sm mt-1">Isi informasi acara dengan lengkap dan akurat untuk pengajuan ke admin</p>
        </div>
      </div>
