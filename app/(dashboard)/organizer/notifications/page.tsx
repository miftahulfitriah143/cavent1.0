...
export default function OrganizerNotificationsPage() {
  ...
  useEffect(() => {
    ...
    // Organizer receives notifications with userId == user.uid
    const q = query(
  ...
  const getIconBg = (type: string, isUnread: boolean) => {
    if (!isUnread) return "bg-gray-50";
    switch (type) {
      case "EVENT_APPROVED": return "bg-emerald-50";
      case "EVENT_REJECTED": return "bg-red-50";
      case "NEW_ATTENDEE": return "bg-blue-50";
    ...
    <div className="max-w-4xl mx-auto pb-20">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-neutral text-sm mt-1">Pembaruan status acara dan informasi penting.</p>
        </div>
      ...
      <div className="space-y-4">
        {notifications.length > 0 ? (
          ...
          <div className="bg-white rounded-3xl p-16 border border-dashed border-gray-200 text-center flex flex-col items-center">
             ...
             <p className="text-neutral text-sm">Pemberitahuan acara Anda akan muncul di sini.</p>
          </div>
        )}
      </div>
    </div>
